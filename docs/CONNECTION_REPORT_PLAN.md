...
| **노드별 공종·공종수** | 노드마다 공종(접속·성단·기타) + 횟수. 기타는 공종명 직접 입력 | 후속 정정 |
...
create type public.connection_task_type as enum ('접속', '성단', '기타');
...
- [ ] **공종 enum** = 접속/성단/기타. 추후 추가 시 마이그레이션. 처음부터 더 넣어둘 게 있나? (예: 측정·통선)
